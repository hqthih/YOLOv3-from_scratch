...
def write_results(prediction, confidence, num_classes, nms_conf = 0.4):
    ...
    for ind in range(batch_size):           #vòng lặp chạy mỗi bức ảnh trong 1 batch 
       ...
       #NMS
        ...
        max_conf, max_conf_score = torch.max(image_pred[:,5:5+ num_classes], 1)  # tìm max confidence class, và index trong từng bounding box (từng hàng 1)
        max_conf = max_conf.float().unsqueeze(1)
        max_conf_score = max_conf_score.float().unsqueeze(1)
        seq = (image_pred[:,:5], max_conf, max_conf_score)  # ví trí bb, pc (max conf ) và class (index của class có confidence cao nhất tại mỗi bb)
        image_pred = torch.cat(seq, 1)
        
        non_zero_ind =  (torch.nonzero(image_pred[:,4]))  # loc cai nao co max_conf den khac 0, return index
        try:
